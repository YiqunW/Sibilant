function [tlikely_ind, plikely_ind, ncandidates, pT_g_max] = process_one_triplet(icomb, Integrals, Params, tripdata, makeplot, plot_dir, labs)	
	...
	[tlikely_ind, plikely_ind, report_ind] = find_Tlikely(pT_g, icomb);
    
    % Report the best odds for a triplet
    pT_g_max = [allodds(report_ind) pT_g(report_ind,:)];
    ...
    if makeplot
        ...
        semilogx(odds_mas_sla_T,4-imax,'.'); 
        axis([1e-10 10*max(allodds) 0 4]); set(gca,'YTick', 1:3, 'YTickLabel', labs([3 2 1])) %plot boundaries
        set(gcf,'color','w');
        print(f, '-r80', '-dpng', strcat(plot_dir, 'topvotes-', num2str(icomb), '.png'));
        
        %plot p(T|{g}) as a function of p(b_i=1)/p(b_i=0)
        f=figure('visible', 'off');
        semilogx(allodds, pT_g, 'LineWidth', 2); 
        legend(labs{1},labs{2},labs{3},'null', 'Location', 'northwest'); 
        axis([min(allodds) max(allodds) -0.05 1.05])
        set(gcf,'color','w');
        print(f, '-r80', '-dpng', strcat(plot_dir, 'pT_g-', num2str(icomb), '.png'));
    end
end


function [tlikely_ind, plikely_ind, report_ind] = find_Tlikely(pT_g, triplet)
    ...
    if isempty(itmaxnot4)
        tlikely_ind = 0;
        plikely_ind = max(m);
        report_ind = find(m==max(m));
        report_ind = report_ind(1);
    else
        tlikely_ind = itmax(itmaxnot4(itnot4));
        plikely_ind = not4;
        if length(itnot4) > 1
            itnot4 = itnot4(1); % make sure it's a single value, just in case.
            disp(strcat('Warn (Triplet ', num2str(triplet), '): Multiple prior odds produce same maximum Tlikely.'));
        end
        report_ind = itnot4;
    end
end
